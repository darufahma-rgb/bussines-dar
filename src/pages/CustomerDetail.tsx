...
export default function CustomerDetail() {
  ...
  const handleComplete = async (interactionId: string) => {
    await supabase.from("interactions").update({ is_completed: true }).eq("id", interactionId);
    queryClient.invalidateQueries({ queryKey: ["interactions", id] });
    queryClient.invalidateQueries({ queryKey: ["dashboard-stats"] });
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            {(customer as any).customer_businesses?.map((cb: any) => (
              ...
              return (
                <div key={i.id} className="p-3 flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-muted-foreground">{typeLabels[i.type]}</span>
                      <span className="text-xs text-muted-foreground font-mono">
                        {format(parseISO(i.created_at), "MMM d, h:mm a")}
                      </span>
                      {i.type === "follow_up" && !i.is_completed && (
                    ...
                    {i.follow_up_date && (
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Follow-up: {format(parseISO(i.follow_up_date), "MMM d, yyyy")}
                      </p>
                    )}
                  </div>
                </div>
